...
(async () => {
    try {
        ...
        await rest.put(
            Routes.applicationGuildCommands(discordData.CLIENT_ID, discordData.GUILD_ID),
            { body: commands }
        );
...
audioPlayer.on('error', (error) => {
    client.channels.fetch(channelTextid).then(channel => channel.send(`error reproduciendo la cancion actual ${error}`));
});

audioPlayer.on('stateChange', (oldState, newState) => {
    if (newState.status === AudioPlayerStatus.Idle && oldState.status !== AudioPlayerStatus.Idle) {
        if (playList[indexSongInPlayList]) {
            client.channels.fetch(channelTextid).then(channel => channel.send(playSong(playList[indexSongInPlayList])));
        }
    }
});
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.commandName === 'stop') {
        ...
        await interaction.reply(`Chale ${interaction.user.username} ha detenido la musica (ง'̀-'́)ง`);
    }
...
const playSong = (song) => {
    audioPlayer.stop();
    let stream = ytdl(song.url, { filter: 'audioonly' }).on("error", e => { console.log(e) });
